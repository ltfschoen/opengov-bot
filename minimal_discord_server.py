#!/usr/bin/env python
"""
Minimal Discord verification server based on Python's built-in HTTP server.
This server is designed to handle Discord's verification requests with minimal code.
"""
...
class MinimalDiscordHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def log_message(self, format, *message_args):
        ...
        sys.stderr.write("\033[92m[MinimalDiscordServer]\033[0m %s - - [%s] %s\n" %
                         (self.address_string(),
...
def run_server():
    """Run the minimal Discord verification server"""
    handler = MinimalDiscordHandler

    # Reuse address to avoid "Address already in use" errors
    socketserver.TCPServer.allow_reuse_address = True

    with socketserver.TCPServer(("", PORT), handler) as httpd:
        print(f"🚀 Starting minimal Discord verification server on port {PORT}")
