...
def run_server(port=None):
    # Use the port from command line args if provided, otherwise use default
    if port is None:
        port = args.port

    # Allow server to reuse the address and support dual-stack sockets
    socketserver.TCPServer.allow_reuse_address = True

    # Create a custom server class that supports dual-stack sockets
    class DualStackServer(socketserver.TCPServer):
        def server_bind(self):
            # This allows the server to bind to both IPv4 and IPv6
            try:
                self.socket.setsockopt(socket.IPPROTO_IPV6, socket.IPV6_V6ONLY, 0)
            except (AttributeError, socket.error) as e:
                print(f"Warning: Could not set IPV6_V6ONLY to 0: {e}")
    ...
    try:
        try:
            ...
            with DualStackServer(("localhost", port), InteractionHandler) as httpd:
                ...
                print(f"http://localhost:{port}/interactions")
            ...
            print(f"IPv6 binding failed: {e}")
            print("Falling back to IPv4 only...")
            with socketserver.TCPServer(("0.0.0.0", port), InteractionHandler) as httpd:
                print(f"✅ Verification server running on port {port} (IPv4 only)")
                print("\n==== SUPPORTED ENDPOINTS =====")
                print(f"http://localhost:{port}/api/interactions (primary)")
                print(f"http://localhost:{port}/interactions")
                print(f"http://localhost:{port}/api/discord/interactions")
                ...
                if not DEBUG_MODE:
                    print("\nTo run in debug mode (bypass signature verification):")
        ...
        if "Address already in use" in str(e):
            ...
            print("\nTo fix this, you can:")
            print(f"1. Kill processes using port {port}:")
            print(f"   lsof -ti :{port} | xargs kill -9 2>/dev/null || true")
            print("2. Use a different port:")
            print(f"   python verify_endpoint.py --port={port+1} --debug")
            print("3. Check if another instance of verify_endpoint.py is running")
        else:
            print(f"\n❌ Error starting server: {e}")
            print("Try using a different port with --port=<port>")
        sys.exit(1)
    return True
